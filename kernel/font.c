...
// Standard VGA 8x16 bitmap font (CP437)
...
// TODO(human): Implement font_draw_char and font_draw_string below
void font_draw_char(struct font *font, char c, int x, int y, uint32_t colour){
    if (!font || !font->font_addr) {
        return;
    }

    uint8_t code = (uint8_t)c;
    if (code < font->first_char_code || code > font->last_char_code) {
        code = (uint8_t)'?';
    }

    uint32_t bytes_per_row = (font->width + 7) / 8;
    uint32_t glyph_index = (uint32_t)(code - font->first_char_code);
    const uint8_t *glyph = font->font_addr + glyph_index * (bytes_per_row * font->height);
    ...
    for (uint32_t row = 0; row < font->height; row++) {
        for (uint32_t col = 0; col < font->width; col++) {
            ...
            uint8_t bit_mask = (uint8_t)(0x80 >> (col % 8));

            if ((glyph[byte_index] & bit_mask) == 0) {
                continue;
            }
...
void font_draw_string(struct font *font, const char *str, int x, int y, uint32_t colour){
    ...
    while (*str) {
        ...
        font_draw_char(font, *str, cursor_x, cursor_y, colour);
        cursor_x += font->width;
        str++;
    }
}
